...
void function RTKSmokeTestBehavior_OnInitialize( rtk_behavior self )
{
	...
	if ( RTKArray_GetCount( expect rtk_array( self.rtkprops.arrayOfStructs ) ) <= 0 )
		return

	rtk_struct subStruct1 = RTKArray_GetStruct( expect rtk_array( self.rtkprops.arrayOfStructs ), 0 )
	rtk_struct subStruct2 = RTKStruct_GetStruct( subStruct1, "testNestedStruct" )

	rtk_panel inst = RTKPanel_Instantiate( $"ui_rtk/test/serial_container_test.rpak", self.GetPanel(), "instance" )
	rtk_panel cln = RTKPanel_Clone( inst, self.GetPanel(), "clone", 0 )
}
