		...
		if ( tierDataList.len() == 1 && tierDataList[0].unlocksAt > 0 )
		{
			if ( GladiatorCardBadge_IsValidStatRef( unlockStatRef ) )
			{
				...
				if ( goalVal == 1 )
					...
					currVal = GladiatorCardBadge_GetStatInt( GetLocalClientPlayer(), unlockStatRef )

				if ( good )
					toolTipData.actionHint1 = format( "%s / %s", FormatAndLocalizeNumber( "1", float(  currVal ), true ), FormatAndLocalizeNumber( "1", goalVal, true ) )
			}
