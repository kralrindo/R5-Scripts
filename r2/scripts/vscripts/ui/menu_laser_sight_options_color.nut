...
void function InitLaserSightOptionsColorPanel( var panel ) 
{
	...
	file.reticleColorBtn = Hud_GetChild( file.panel, "BtnReticleColor" )
	...
	AddButtonEventHandler( file.reticleColorBtn, UIE_CLICK, OnReticleColorButtonClicked )
...
void function OnShowPanel( var panel )
{
	...
	file.changingColorNotUsingSliders = true
	{
		...
		UpdateLaserSightColorOptions( file.currentColor )
		...
		SetLaserSightOptionsMenuColor( file.currentColor )
...
void function OnReticleColorButtonClicked( var btn )
{
	file.changingColorNotUsingSliders = true
	UpdateColor(file.reticleColor, false, true)
}
...
void function UpdateColor(vector color, bool validate = false, bool calculateSliderProgress = false)
{
	...
	{
		if( validate )
			color = ColorPalette_ClampAndValidateLaserSightColor( color )
...
{
	file.currentColor = ColorPalette_ClampAndValidateLaserSightColor( file.currentColor )

	SetConVarColor( "laserSightColor", file.currentColor )
...
void function UpdateLaserSightColorOptions( vector currentColor )
{
	RuiSetFloat3(  Hud_GetRui( file.defaultColorBtn ), "paletteColor", file.defaultColor )
	RuiSetFloat3(  Hud_GetRui( file.reticleColorBtn ), "paletteColor", file.reticleColor )
...
void function RGB_OnChanged( var button )
{
	...
	UpdateLaserSightColorOptions( file.manualColor )
...
void function OnResetColorButtonClicked( var btn )
{
	if(OptionsColor_HasColorChanged( file.previousColor, file.currentColor ))
	{
		...
		data.headerText = "#REVERT_CUSTOM_LASER_CHANGES"
...
{
	ConfirmDialogData data
	data.headerText = "#RESTORE_CUSTOM_LASER_DEFAULTS"
	data.messageText = "#RESTORE_CUSTOM_LASER_DEFAULTS_DESC"
