...
void function CustomMatch_RefreshPlaylists()
{
	...
	if ( file.activeCategory.maps.len() > 0 )
	{
		...
		{
			if ( category.displayName == file.activeCategory.displayName )
			{
...
string function UICodeCallback_CustomMatchGetPlayerUidFromHash( string hardware, string uidHashed )
{
	for( int i = 0; i < file.teams.len(); i++ )
	{
		foreach( CustomMatch_LobbyPlayer player in file.teams[i] )
		{
			if( uidHashed == player.uidHashed && hardware == player.hardware )
			{
				return player.uid
			}
		}
...
void function UICodeCallback_CustomMatchGetPlayers()
{
	...
	CustomMatch_LiveAPI_SendPlayers(players)
}
