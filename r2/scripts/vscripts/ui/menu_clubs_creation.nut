...
global function ClubCreationDoingEdit
...
const int CLUB_NAME_CHAR_LIMIT = 16
...
void function InitClubsCreationMenu( var menu )
{
	...
	HudElem_SetRuiArg( Hud_GetChild( file.settingsPanel, "ClubNameTextEntryFrame" ), "subtitle", "#CLUB_CREATION_CLUBNAME_DESC" )
	...
	AddButtonEventHandler( file.clubNameField, UIE_SPECCHAR, ClubName_OnSpecialCharacter )
	...
	AddButtonEventHandler( file.clubTagField, UIE_SPECCHAR, ClubName_OnSpecialCharacter )
...
{
	if ((Hud_GetUTF8Text( button )).len() >= CLUB_NAME_CHAR_LIMIT)
	{
		SetClubSubtitleDefault();
	}
	SetChangesMade()
}
...
void function ClubName_OnSpecialCharacter( var button )
{
	if ((Hud_GetUTF8Text( button )).len() < CLUB_NAME_CHAR_LIMIT)
	{
	HudElem_SetRuiArg( Hud_GetChild( file.settingsPanel, "ClubNameTextEntryFrame" ), "subtitle", "#CLUB_CREATION_CLUBNAME_SPECCHAR")
	}
}

void function SetClubSubtitleDefault()
{
	file.menu = GetMenu( "ClubsCreationMenu" )
	file.settingsPanel = Hud_GetChild( file.menu, "ClubCreationSettingsPanel" )
	HudElem_SetRuiArg( Hud_GetChild( file.settingsPanel, "ClubNameTextEntryFrame" ), "subtitle", "#CLUB_CREATION_CLUBNAME_DESC" )
}
...
bool function ClubCreationDoingEdit()
{
   return file.doingEdit
}
void function ConfigureMenuForCreation()
{
	file.doingEdit = false;
	RuiSetString( file.menuHeaderRui, "menuName", "#LOBBY_CLUBS_CREATE" )
	SetClubSubtitleDefault()
...
void function TryOpenChangeConfirmationDialog( bool commitChange )
{
	...
	if( !ClubCreationDoingEdit() && ClubIsValid() )
	{	
		...
		if ( GetActiveMenu() == file.menu )
	    	...
	    	Clubs_OpenClubCreateBlockedByJoinDialog()
		Clubs_UpdateMyData()
		return
	}
