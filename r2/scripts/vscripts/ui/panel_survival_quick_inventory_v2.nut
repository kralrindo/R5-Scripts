...
void function OnEquipmentButtonClickRight( var button )
{
	...
	if ( IsFullyConnected() )
	{
		...
		RunClientScript( "UICallback_OnEquipmentButtonAction", button, eLootActionType.ALT_ACTION, false )
	}
}
...
bool function OnEquipmentKeyPress( var button, int keyId, bool isDown )
{
	...
	if ( keyId == BUTTON_SHOULDER_LEFT )
	{
		...
		{
			EmitUISound( "UI_InGame_Inventory_Select" )
			RunClientScript( "UICallback_OnEquipmentButtonAction", button, eLootActionType.PRIMARY_SWAP, false )
		}
		return true
	}
	...
	if( keyId == BUTTON_TRIGGER_LEFT )
	{
		...
		{
			EmitUISound( "UI_InGame_Inventory_Select" )
			RunClientScript( "UICallback_OnEquipmentButtonAction", button, eLootActionType.CHARACTER_ACTION1, false )
		}
		return true
	}
	...
	if( keyId == BUTTON_TRIGGER_RIGHT )
	{
		...
		{
			EmitUISound( "UI_InGame_Inventory_Select" )
			RunClientScript( "UICallback_OnEquipmentButtonAction", button, eLootActionType.CHARACTER_ACTION2, false )
		}
		return true
	}
...
void function SurvivalInventoryMenu_SetSpaceForSling( bool hasSling )
{
		...
		int weaponOffset = hasSling ? -110 : -80

		var weapon0 = Hud_GetChild( file.mainInventoryPanel, "MainWeapon0" )
		Hud_SetPos( weapon0, Hud_GetX(weapon0 ), ContentScaledYAsInt( weaponOffset ) )

		var weapon1 = Hud_GetChild( file.mainInventoryPanel, "MainWeapon1" )
		Hud_SetPos( weapon1, Hud_GetX(weapon1 ), ContentScaledYAsInt( weaponOffset ) )
