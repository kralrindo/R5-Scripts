...
void function InviteAllClubMembersToParty()
{
	...
	{
		if ( !(member.memberID in file.memberOnlineStatus) )
			continue
...
void function HideClubsWarning_Thread( var commBlockWarning )
{
	...
	Assert( false == file.commBlockWarningThread )
...
void function UpdateClubChat()
{
	...
	if ( UITime() < file.commBlockWarningFadeTime )
	{
		...
		if ( !file.commBlockWarningThread )
			thread HideClubsWarning_Thread( commBlockWarning )
	}
	else
	{
		Hud_SetHeight( commBlockWarning, 0 )
		RuiSetInt( Hud_GetRui( commBlockWarning ), "ruiStyle", 0 )
	}
...
void function QueryClubTabsForNewness()
{
	...
	float currentFadeTime = UITime() + CommBlockSecondsToWarnFade()
	file.commBlockWarningId = GetCommunicationBlockWarningID()

	
	if ( prevStateID != file.commBlockWarningId || file.commBlockWarningFadeTime + 0.25 < currentFadeTime )
	{
		var commBlockWarning = Hud_GetChild( file.chatPanel, "CommBlockWarning" )
		var commBlockRui = Hud_GetRui( commBlockWarning )
		...
		printf( "[commBlock] QueryClubTabsForNewness YEP old = %d %0.2f, new = %d %0.2f", prevStateID, file.commBlockWarningFadeTime, file.commBlockWarningId, currentFadeTime )

		
		if ( file.commBlockWarningId <= 0 )
			file.commBlockWarningFadeTime = 0
		else
			file.commBlockWarningFadeTime = currentFadeTime
		UpdateClubChat()
	}
	else
	{
		printf( "[commBlock] QueryClubTabsForNewness NOPE old = %d %0.2f, new = %d %0.2f", prevStateID, file.commBlockWarningFadeTime, file.commBlockWarningId, currentFadeTime )
	}
